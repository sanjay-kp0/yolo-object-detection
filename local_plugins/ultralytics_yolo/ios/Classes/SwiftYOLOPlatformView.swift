...
public class SwiftYOLOPlatformView: NSObject, FlutterPlatformView, FlutterStreamHandler {
  ...
  init(
    ...
    print("📡 SwiftYOLOPlatformView.init: Creating event channel with name: \(eventChannelName)")
    ...
    print("📡 SwiftYOLOPlatformView.init: Creating method channel with name: \(methodChannelName)")
    ...
    print("📡 SwiftYOLOPlatformView.init: Setting stream handler for event channel")
    self.eventChannel.setStreamHandler(self)
    print("✅ SwiftYOLOPlatformView.init: Event channel configured, waiting for Flutter to listen...")
  ...
  private func setupYOLOViewStreaming(args: [String: Any]) {
    print("🔧 SwiftYOLOPlatformView.setupYOLOViewStreaming: Setting up streaming...")
    guard let yoloView = yoloView else {
      print("❌ SwiftYOLOPlatformView.setupYOLOViewStreaming: yoloView is nil!")
      return
    }
    ...
    if let configDict = streamingConfigParam {
      print("🔧 SwiftYOLOPlatformView.setupYOLOViewStreaming: Using custom stream config")
      streamConfig = YOLOStreamConfig.from(dict: configDict)
    } else {
      // Use default minimal configuration for optimal performance
      print("🔧 SwiftYOLOPlatformView.setupYOLOViewStreaming: Using DEFAULT stream config")
    ...
    print("✅ SwiftYOLOPlatformView.setupYOLOViewStreaming: Stream config applied")

    // Set up streaming callback to forward data to Flutter via event channel
    yoloView.setStreamCallback { [weak self] streamData in
      print("🎯 SwiftYOLOPlatformView: Stream callback triggered!")
      self?.sendStreamDataToFlutter(streamData)
    }
    print("✅ SwiftYOLOPlatformView.setupYOLOViewStreaming: Stream callback registered!")
  }

  /// Send stream data to Flutter via event channel
  private func sendStreamDataToFlutter(_ streamData: [String: Any]) {
    print("📤 SwiftYOLOPlatformView.sendStreamDataToFlutter: Attempting to send data...")
    guard let eventSink = self.eventSink else {
      print("❌ SwiftYOLOPlatformView.sendStreamDataToFlutter: eventSink is NIL! No listener connected.")
      return
    }
    print("✅ SwiftYOLOPlatformView.sendStreamDataToFlutter: eventSink available, sending data with keys: \(streamData.keys)")
    DispatchQueue.main.async {
      eventSink(streamData)
      print("✅ SwiftYOLOPlatformView.sendStreamDataToFlutter: Data sent successfully!")
    }
  }
  ...
  public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink)
    -> FlutterError?
  {
    print("🎧 SwiftYOLOPlatformView.onListen: Flutter listener CONNECTED to event channel")
    self.eventSink = events
    print("✅ SwiftYOLOPlatformView.onListen: eventSink set successfully, ready to send events!")
    return nil
  }

  public func onCancel(withArguments arguments: Any?) -> FlutterError? {
    print("🔌 SwiftYOLOPlatformView.onCancel: Flutter listener DISCONNECTED from event channel")
    self.eventSink = nil
    return nil
  }
