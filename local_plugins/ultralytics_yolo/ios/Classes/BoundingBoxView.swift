...
class BoundingBoxView {
  ...
  func show(frame: CGRect, label: String, color: UIColor, alpha: CGFloat) {
    ...
    // Calculate circle that encompasses the bounding box
    let centerX = frame.midX
    let centerY = frame.midY
    let diameter = max(frame.width, frame.height) * 1.1  // Slightly larger than box
    ...
    let path = UIBezierPath(ovalIn: circleRect)  // Circle instead of rectangle
