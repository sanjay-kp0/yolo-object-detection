...
class ObjectDetector: BasePredictor {
  ...
  override func processObservations(for request: VNRequest, error: Error?) {
    print("🔬 ObjectDetector.processObservations: Called!")
    print("🔬 ObjectDetector.processObservations: Error: \(error != nil ? String(describing: error) : "nil")")
    print("🔬 ObjectDetector.processObservations: Results type: \(type(of: request.results))")
    
    if let results = request.results as? [VNRecognizedObjectObservation] {
      print("🔬 ObjectDetector.processObservations: Got \(results.count) observations")
      var boxes = [Box]()

      // Limit the number of detections based on numItemsThreshold
      let maxDetections = min(results.count, self.numItemsThreshold)
      print("🔬 ObjectDetector.processObservations: Processing \(maxDetections) detections (threshold: \(self.numItemsThreshold))")
      ...
      print("✅ ObjectDetector.processObservations: Created result with \(boxes.count) boxes")
      print("✅ ObjectDetector.processObservations: currentOnResultsListener is \(self.currentOnResultsListener != nil ? "SET" : "NIL")")
      print("✅ ObjectDetector.processObservations: Calling currentOnResultsListener.on(result:)...")
      self.currentOnResultsListener?.on(result: result)
      print("✅ ObjectDetector.processObservations: Callback completed!")

    } else {
      print("❌ ObjectDetector.processObservations: Could NOT cast results to [VNRecognizedObjectObservation]")
    }
  }
