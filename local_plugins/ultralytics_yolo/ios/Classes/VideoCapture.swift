...
class VideoCapture: NSObject, @unchecked Sendable {
  ...
  private func predictOnFrame(sampleBuffer: CMSampleBuffer) {
    guard let predictor = predictor else {
      print("⚠️ VideoCapture.predictOnFrame: predictor is nil")
      return
    }
    if frameCounter % 60 == 1 {
      print("🔮 VideoCapture.predictOnFrame: Starting prediction on frame #\(frameCounter)")
    }
    if currentBuffer == nil, let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
      ...
      if !frameSizeCaptured {
        ...
        print("📐 CAMERA FRAME SIZE: \(frameWidth) x \(frameHeight)")
      ...
      if frameCounter % 60 == 1 {
        print("🔮 VideoCapture.predictOnFrame: Calling predictor.predict...")
      }
      predictor.predict(sampleBuffer: sampleBuffer, onResultsListener: self, onInferenceTime: self)
      if frameCounter % 60 == 1 {
        print("✅ VideoCapture.predictOnFrame: predictor.predict called successfully")
      }
      currentBuffer = nil
    }
  }
...
extension VideoCapture: AVCaptureVideoDataOutputSampleBufferDelegate {
  func captureOutput(
    ...
    frameCounter += 1
    // Log every 60th frame to avoid spam (~ every 2 seconds at 30fps)
    if frameCounter % 60 == 1 {
      print("📹 VideoCapture.captureOutput: Frame #\(frameCounter) received, inferenceOK: \(inferenceOK), predictor exists: \(predictor != nil)")
    }
    guard inferenceOK else {
      if frameCounter % 60 == 1 {
        print("❌ VideoCapture.captureOutput: inferenceOK is FALSE, skipping frame")
      }
      return
    }
...
extension VideoCapture: ResultsListener, InferenceTimeListener {
  ...
  func on(result: YOLOResult) {
    print("🎯 VideoCapture.on(result:): Received result with \(result.boxes.count) boxes")
    print("🎯 VideoCapture.on(result:): Delegate is \(self.delegate != nil ? "SET" : "NIL")")
    DispatchQueue.main.async {
      print("🎯 VideoCapture.on(result:): Calling delegate.onPredict...")
      self.delegate?.onPredict(result: result)
      print("✅ VideoCapture.on(result:): delegate.onPredict called")
    }
  }
}
