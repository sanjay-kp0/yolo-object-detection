...
public class YOLOView: UIView, VideoCaptureDelegate {
  ...
  func onPredict(result: YOLOResult) {
    print("🎯 YOLOView.onPredict: Detection received with \(result.boxes.count) boxes")

    // Check if we should process inference result based on frequency control
    if !shouldRunInference() {
      print("⏭️ YOLOView.onPredict: Skipped (inference frequency control)")
      return
    }
  ...
  public init(
    ...
    print("🚀 YOLOView.init: Initializing with model='\(modelPathOrName)', task=\(task)")
    self.videoCapture = VideoCapture()
    super.init(frame: frame)
    print("🚀 YOLOView.init: Calling setModel...")
    setModel(modelPathOrName: modelPathOrName, task: task)
    setUpOrientationChangeNotification()
    self.setUpBoundingBoxViews()
    self.setupUI()
    self.videoCapture.delegate = self
    print("🚀 YOLOView.init: Setup complete, delegate set")
  ...
  public func setModel(
    ...
    print("🔵 YOLOView.setModel: Called with modelPath='\(modelPathOrName)', task=\(task)")
    ...
    print("🔍 YOLOView.setModel: Searching for model '\(modelPathOrName)'...")

    // Determine model URL
    if lowercasedPath.hasSuffix(".mlmodel") || lowercasedPath.hasSuffix(".mlpackage")
      || lowercasedPath.hasSuffix(".mlmodelc")
    {
      print("🔍 YOLOView.setModel: Model path has extension, checking file system...")
      let possibleURL = URL(fileURLWithPath: modelPathOrName)
      var isDirectory: ObjCBool = false
      if fileManager.fileExists(atPath: possibleURL.path, isDirectory: &isDirectory) {
        modelURL = possibleURL
        print(
          "✅ YOLOView.setModel: Found model at: \(possibleURL.path) (isDirectory: \(isDirectory.boolValue))")
      } else {
        print("❌ YOLOView.setModel: Model NOT found at path: \(possibleURL.path)")
      }
    } else {
      print("🔍 YOLOView.setModel: Searching in app bundle for '\(modelPathOrName)'...")
      if let compiledURL = Bundle.main.url(forResource: modelPathOrName, withExtension: "mlmodelc")
      {
        print("✅ YOLOView.setModel: Found compiled model (.mlmodelc) at: \(compiledURL.path)")
        modelURL = compiledURL
      } else if let packageURL = Bundle.main.url(
        forResource: modelPathOrName, withExtension: "mlpackage")
      {
        print("✅ YOLOView.setModel: Found package model (.mlpackage) at: \(packageURL.path)")
        modelURL = packageURL
      } else {
        print("❌ YOLOView.setModel: Model NOT found in bundle with name '\(modelPathOrName)'")
      }
    }
    ...
    guard let unwrappedModelURL = modelURL else {
      ...
      print(
        "⚠️ YOLOView.setModel: Model file not found: \(modelPathOrName). Camera will run without inference."
      )
    ...
    print("✅ YOLOView.setModel: Using model URL: \(unwrappedModelURL.path), modelName: \(modelName)")

    // Common success handling for all tasks
    func handleSuccess(predictor: Predictor) {
      print("✅ YOLOView.setModel: Predictor created successfully for task: \(task)")
      ...
      print("✅ YOLOView.setModel: Predictor assigned to videoCapture")
      ...
      if let basePredictor = predictor as? BasePredictor {
        ...
        print("✅ YOLOView.setModel: Stream config applied to predictor")
      }

      self.activityIndicator.stopAnimating()
      self.labelName.text = modelName
      print("✅ YOLOView.setModel: Model loaded successfully! Ready to detect.")
      completion?(.success(()))
    }

    // Common failure handling for all tasks
    func handleFailure(_ error: Error) {
      print("❌ YOLOView.setModel: Failed to load model with error: \(error)")
      self.activityIndicator.stopAnimating()
    ...
    default:
      print("🔧 YOLOView.setModel: Creating ObjectDetector for detection task...")
      ObjectDetector.create(unwrappedModelURL: unwrappedModelURL, isRealTime: true) {
        [weak self] result in
        print("🔧 YOLOView.setModel: ObjectDetector.create completion called")
        switch result {
        case .success(let predictor):
          print("✅ YOLOView.setModel: ObjectDetector created successfully")
          handleSuccess(predictor: predictor)
        case .failure(let error):
          print("❌ YOLOView.setModel: ObjectDetector creation failed")
          handleFailure(error)
        }
      }
    }
      ...
      videoCapture.setUp(sessionPreset: .hd1280x720, position: position, orientation: orientation) {
        ...
        if success {
          ...
          if let previewLayer = self.videoCapture.previewLayer {
            ...
            print("✅ YOLOView.start: Preview layer added")
          }
          ...
          print("❌ YOLOView.start: Video capture setup FAILED")
          self.busy = false
        }
      }
    } else {
      print("⚠️ YOLOView.start: Already busy, skipping start")
    }
  }
...
extension YOLOView: AVCapturePhotoCaptureDelegate {
  ...
  private func convertResultToStreamData(_ result: YOLOResult) -> [String: Any] {
    ...
    // Always include frame size for coordinate mapping
    map["frameWidth"] = Double(result.orig_shape.width)
    map["frameHeight"] = Double(result.orig_shape.height)
