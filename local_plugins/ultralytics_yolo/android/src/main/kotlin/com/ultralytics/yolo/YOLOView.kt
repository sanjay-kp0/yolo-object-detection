...
class YOLOView @JvmOverloads constructor(
            ...
            when (task) {
                ...
                YOLOTask.DETECT -> {
                    ...
                    for (box in result.boxes) {
                        ...
                        Log.d(TAG, "Drawing circle for ${box.cls}: L=$left, T=$top, R=$right, B=$bottom, conf=${box.conf}")
                        ...
                        val circleCenterX = (left + right) / 2
                        val circleCenterY = (top + bottom) / 2
                        val diameter = maxOf(right - left, bottom - top) * 1.1f  // Slightly larger
    ...
    private fun convertResultToStreamData(result: YOLOResult): Map<String, Any> {
        ...
        // Always include frame size for coordinate mapping (dynamic for all devices)
        map["frameWidth"] = result.origShape.width.toDouble()
        map["frameHeight"] = result.origShape.height.toDouble()
